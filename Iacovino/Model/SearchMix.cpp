...
// May 13 2013 fumarole data at Cerro Negro. T = 165degC
#if 1
double surfaceCO[3] = {0.0000, 0.0, 0.0};
double surfaceCO2[3] = {0.2662, 0.0, 0.0};
double surfaceH2O[3] = {0.7304, 0.0, 0.0};
double surfaceH2S[3] = {0.0004, 0.0, 0.0};
double surfaceSO2[3] = {0.0029, 0.0, 0.0};
#else // #TODO - Rick, what is this else statement doing?
double surfaceCO2[3] = {12.9, 0.0, 0.0};
double surfaceH2O[3] = {86.9, 0.0, 0.0};
double surfaceH2S[3] = {0.02, 0.0, 0.0};
double surfaceSO2[3] = {1.30, 0.0, 0.0};
#endif
...
SearchMix::search()
{
	...
	fprintf(ofp,"\n\nDeep   Shallow  D->S          %10.10s    %10.10s    %10.10s    %10.10s    %10.10s\n",
													"CO", "CO2", "H2O", "H2S", "SO2");
	fprintf(ofp,"----  ----  ----           %10.10s    %10.10s    %10.10s    %10.10s    %10.10s\n",
													"--", "---", "---", "---", "---");
	//Generates a list of one-dimensional arrays, each array 3 values long, where the sum of each array is 100
	//for (int a=0;a<100;a++)
	...
	^(size_t inA)
	{
		...
		for (int b=100-a; b>-1; b--)
		{
			for (int c=100-a-b; c>-1; c--)
			{
...
#if 1
								double co = getVals(a,b,c, mCOVals);
								if ((co < surfaceCO[1]) || (co >surfaceCO[2]))
								{
									continue;
								}
#endif
								double co2 = getVals(a,b,c, mCO2Vals);
								if ((co2 < surfaceCO2[1]) || (co2 >surfaceCO2[2]))
								{
									continue;
								}

								double h2o = getVals(a,b,c, mH2OVals);
								if ((h2o < surfaceH2O[1]) || (h2o >surfaceH2O[2]))
								{
									continue;
								}

								double h2s = getVals(a,b,c, mH2SVals);
								if ((h2s < surfaceH2S[1]) || (h2s >surfaceH2S[2]))
								{
									continue;
								}

								double so2 = getVals(a,b,c, mSO2Vals);
								...
								fprintf(ofp,"%4d  %4d  %4d  = %4d - %10.7f    %10.7f    %10.7f    %10.7f    %10.7f\n",a,b,c, sum, co, co2, h2o, h2s, so2);
							}
						}
...
{
	std::printf("\n\n");
	std::printf("Calculated equilibrium and degassing fluids:\n");

	std::printf("      %15.15s  %15.15s  %15.15s\n",
						"Deep", "Shallow", "D->S");
	std::printf("      %15.15s  %15.15s  %15.15s\n",
						"---------------", "---------------", "---------------");
